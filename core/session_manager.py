...
class SessionManager:
    ...
    def check_session(self) -> Dict:
        ...
        try:
            ...
            settings.write_log_event( "session_validation_failed", "ERROR", reason="read_or_decrypt_error",  exception_type=type(e).__name__ , error=str(e))
    ...
    def create_session(  self,  username: str, password: str,  p_p_entity_Origine: str, p_entity_New: str,  Id_USER, ) -> bool:
        try:
            ...
            with open(self.session_path, "w", encoding="utf-8") as f:
                f.write(encrypted)
            settings.write_log_event( "session_created", "INFO", has_username=bool(username),  has_entity=bool(p_entity_New),  session_age_seconds=int((datetime.datetime.now(self.timezone) - now).total_seconds()) )
            return True
        except Exception as e:
            settings.write_log_event( "session_creation_failed", "ERROR",  exception_type=type(e).__name__ , error=str(e) )
            return False
    ...
    def clear_session(self):
        if ValidationUtils.pathExists(self.session_path):
            try:
                ...
                settings.write_log_event(  "session_clear_failed", "ERROR",  exception_type=type(e).__name__ , error=str(e)  )
        else:
    ...
    def validate_session_with_api(self, username: str, p_entity: str) -> Dict:
        try:
            ...
            try:
                ...
                settings.write_log_event(  "session_api_validation_failed","ERROR", reason="decryption_error",  exception_type=type(e_decrypt).__name__,  error=str(e_decrypt) )
        ...
        try:
            ...
            for attempt in range(1, 6):
                ...
                try:
                    ...
                    if resp is not None:
                        settings.write_log_event( "credentials_api_response_received","DEBUG",  attempt=attempt, response_size=len(str(resp)) if resp is not None else 0)
                        break
                except Exception as e:
            ...
            try:
                ...
                settings.write_log_event("credentials_response_decryption_failed","ERROR", exception_type=type(e).__name__ ,  error=str(e) )
                return -5
        except Exception as e:
