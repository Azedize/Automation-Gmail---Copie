...
class ApiClient:
    ...
    def makeRequest( self,  endpoint: str, method: str = "POST",  data: Optional[Dict] = None,  json_data: Optional[Dict] = None, params: Optional[Dict] = None,  headers: Optional[Dict] = None, timeout: int = 30 ) -> Dict[str, Any]:
        url = ( Settings.API_ENDPOINTS.get(endpoint, endpoint)
            ...
            try:
                ...
                if response.status_code == 200:
                    try:
                        ...
                        Settings.write_log_event(
                            "http_json_decode_failed", "WARNING"
                        )
                        return {
                            "status": "success",
                            "data": response.text,
                ...
                # 🔒 Auth / Accès refusé
                    ...
                    return {
                        "status": "error",
                        "error": f"HTTP {response.status_code}: Access denied / session expired",
                        "status_code": response.status_code,
                    }
        ...
        return {
            "status": "error",
            "error": f"Failed after 5 attempts: {last_exception}",
            "status_code": None,
        }
    ...
    def saveProcess(self, params: Dict[str, Any]) -> int:
        ...
        if isinstance(data, dict) and data.get("status") is True:
            Settings.write_log_event(
                "process_saved",
                "INFO",
                has_inserted_id=bool(data.get("inserted_id")),
            )
        ...
        try:
            ...
            if missing:
                ...
                return {
                    "valid": False,
                    "data": data,
                    "error": "Données du service non valides : certaines adresses IP attendues sont absentes de la réponse. Veuillez réessayer ou contacter le support si le problème persiste.",
                }

            Settings.write_log_event(
                "proxy_configuration_fetch_completed",
                "INFO",
                ip_count=len(api_ips),
            )
