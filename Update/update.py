...
class UpdateManager:
    ...
    def checkAndUpdate(window=None) -> bool:
        ...
        settings.write_log_dev_file(f"Date de session utilisée pour la vérification du programme: {session_date_plain}", "INFO")
        settings.write_log_dev_file("Session de programme valide: type datetime détecté.", "INFO")
        ...
        try:
            ...
            settings.write_log_dev_file(f"Date chiffrée pour le check programme: {encrypted_safe}", "INFO")

        except Exception as e:
            Settings.write_log_dev_file( f"Échec du chiffrement pour la vérification du programme : {e}\n{traceback.format_exc()}", "ERROR" )
            traceback.print_exc()
            return False
        ...
        settings.write_log_dev_file(f"URL finale API programme: {CHECK_URL_PROGRAMM}", "INFO")
        settings.write_log_dev_file(f"URL de téléchargement programme: {SERVER_ZIP_URL_PROGRAM}", "DEBUG")

  
        try:
            settings.write_log_dev_file("Vérification de mise à jour du programme en cours...", "INFO")
            response = API_MANAGER.makeRequest(CHECK_URL_PROGRAMM, method="GET", timeout=10)

            settings.write_log_dev_file(f"Réponse brute API programme: {response}", "DEBUG")
            settings.write_log_dev_file(f"Type de la réponse API programme: {type(response)}", "DEBUG")

            if not isinstance(response, dict) or response.get("status_code") != 200:
                settings.write_log_dev_file(f"Serveur programme indisponible: {response}", "WARNING")
                settings.write_log_dev_file("Aucune mise à jour du programme appliquée; on continue sans changement.", "WARNING")
                return False
            ...
            if not local_program or local_program != server_program:
                Settings.write_log_dev_file(f"Mise à jour du programme nécessaire: local={local_program}, remote={server_program}", "INFO")

                if window and hasattr(window, "close"):
                    settings.write_log_dev_file("Fermeture de la fenêtre avant relancement du programme d'installation.", "DEBUG")
                    window.close()

                UpdateManager.launchNewWindow()
                settings.write_log_dev_file("Relancement du programme déclenché après différence de version.", "INFO")
                return True
            Settings.write_log_dev_file(f"Programme à jour: local={local_program}, remote={server_program}", "INFO")
            Settings.write_log_dev_file("=== CHECK PROGRAM VERSION END ===", "INFO")
            return True
        except ImportError:
            Settings.write_log_dev_file("API_MANAGER non disponible lors du check du programme.", "INFO")
            return False
        except Exception as e:
            Settings.write_log_dev_file( f"Erreur critique lors de la vérification de mise à jour du programme: {e}\n{traceback.format_exc()}",  "ERROR" )
            traceback.print_exc()
            return False
    ...
    def _find_version_in_extension_root(extensions_root, target_name):
        ...
        if not extensions_root.is_dir():
            Settings.write_log_dev_file(f"Dossier extensions introuvable: {extensions_root}", "DEBUG")
            return None
        ...
        try:
            for extension_dir in extensions_root.iterdir():
                ...
                version_dirs = [path for path in extension_dir.iterdir() if path.is_dir()]
                for version_dir in version_dirs:
                    ...
                    try:
                        ...
                        Settings.write_log_dev_file(f"Manifest extension illisible: {manifest_path}; erreur={e}", "WARNING")
                        continue
    ...
    def _find_firefox_extension_version(profile_dir, target_id):
        ...
        try:
            for extension_file in extensions_dir.iterdir():
                if extension_file.suffix.lower() == ".xpi":
                    try:
                        ...
                        Settings.write_log_dev_file(f"XPI Firefox illisible: {extension_file}; erreur={e}", "WARNING")
                        continue
            ...
            Settings.write_log_dev_file(f"Erreur accès extensions Firefox {extensions_dir}: {e}", "ERROR")
    ...
    def getInstalledBrowserExtensionVersion(browser_name, target_name="EX3"):
        ...
        candidates = [
            ...
            for relative_path in Settings.BROWSER_EXTENSION_CANDIDATES.get(browser_name, ())
        ]

        for candidate in candidates:
            Settings.write_log_dev_file(f"Recherche extension dans le dossier navigateur: {candidate}", "DEBUG")
            if browser_name == "firefox":
                if not candidate.exists():
                    Settings.write_log_dev_file(f"Dossier Firefox non trouvé: {candidate}", "DEBUG")
                    continue
                for profile_dir in candidate.iterdir():
                    if not profile_dir.is_dir():
                        continue
                    Settings.write_log_dev_file(f"Scan du profil Firefox: {profile_dir}", "DEBUG")
                    version = UpdateManager._find_firefox_extension_version(profile_dir, target_name)
                    if version:
                        return version
                    for extension_root in (profile_dir / "extensions", profile_dir):
                        ...
                        if version:
                            Settings.write_log_dev_file(f"Version extension {target_name} détectée dans Firefox via {extension_root}: {version}", "INFO")
                            return version
                continue
    ...
    def validateBrowserExtensionVersion(browser_name, remote_version, target_name="EX3", window=None):
        ...
        if installed_version is None:
            Settings.write_log_dev_file(f"Extension {target_name} non trouvée pour le navigateur {browser_name} lors de la validation.", "WARNING")
            if window:
                UIManager.showCriticalMessage( window,  "Extension not detected",  f"The {target_name} extension could not be found in {browser_name}. Please contact support to validate the installation before continuing.",  message_type="warning"  )
            return False

        if str(installed_version) != str(remote_version):
            Settings.write_log_dev_file(  f"Version de l'extension {target_name} différente pour {browser_name}: local={installed_version}, remote={remote_version}",  "WARNING"  )
            if window:
                UIManager.showCriticalMessage(  window, "Extension update required",  f"The {target_name} extension is outdated on {browser_name}.\n\nInstalled version: {installed_version}\nCurrent version: {remote_version}\n\nPlease contact support to complete the extension update. The application itself will not be modified.",  message_type="warning" )
            Settings.write_log_dev_file("=== VALIDATION VERSION EXTENSION END (MISMATCH) ===", "INFO")
            return False

        Settings.write_log_dev_file( f"Extension {target_name} validée pour {browser_name}: local={installed_version}, remote={remote_version}", "INFO" )
        Settings.write_log_dev_file("=== VALIDATION VERSION EXTENSION END (OK) ===", "INFO")
        return True
    ...
    def checkExtensionVersion(window=None, browser_name=None):
        ...
        Settings.write_log_dev_file(f"Paramètres check extension: browser_name={browser_name}, window={bool(window)}", "INFO")
        ...
        session_date_plain = session_dt.strftime("%Y-%m-%d")
        settings.write_log_dev_file(f"Session date utilisée pour l’extension (format YYYY-MM-DD): {session_date_plain}", "INFO")
        ...
        try:
            date_encrypted = EncryptionService.encrypt_message(session_date_plain, Settings.KEY)
            settings.write_log_dev_file(f"Date chiffrée pour le check extension: {date_encrypted}", "INFO")
        except Exception as e:
            settings.write_log_dev_file(f"Échec du chiffrement pour le check extension: {e}\n{traceback.format_exc()}", level="ERROR")
            traceback.print_exc()
            return False
        ...
        try:
            ...
            settings.write_log_dev_file(f"Échec de récupération de la version distante de l’extension: {e}\n{traceback.format_exc()}", level="ERROR")
            traceback.print_exc()
            if window:
        ...
        settings.write_log_dev_file(f"Navigateurs à vérifier pour l’extension: {browser_candidates}", "INFO")

        local_version = None
        detected_browser = None
        for candidate in browser_candidates:
            version = UpdateManager.getInstalledBrowserExtensionVersion(candidate, Settings.EXTENSION_TARGET_NAME)
            if version:
                local_version = version
                detected_browser = candidate
                settings.write_log_dev_file(f"Version locale extension détectée avec {candidate}: {local_version}", "INFO")
                break
        ...
        if str(local_version) != str(remote_version):
            settings.write_log_dev_file(f"Mise à jour de l’extension requise: local={local_version}, remote={remote_version}", "INFO")
            settings.write_log_dev_file("=== CHECK EXTENSION VERSION END (VERSION MISMATCH) ===", "INFO")
            return remote_version
        settings.write_log_dev_file(f"Extension à jour: local={local_version}, remote={remote_version}", "INFO")
