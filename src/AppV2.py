...
class AutomationMainWindow(QMainWindow):
    ...
    def submit_button_clicked(self, window):
        ...
        Settings.write_log_dev_file(
            "Start clicked: closing AppV2 and launching checkV3 for the mandatory update check.",
            "INFO",
        )
        if not UpdateManager.launchNewWindow():
            Settings.write_log_dev_file(
                "Unable to launch checkV3.py from Start.", "ERROR"
            )
            UIManager.showCriticalMessage(
                self,
                "Mise à jour indisponible",
                "Impossible de lancer le gestionnaire de mise à jour. L’application reste ouverte.",
                message_type="critical",
            )
            UIManager.enableButton(self.submitButton)
            return
        ...
        Settings.write_log_dev_file(
            "checkV3.py launched successfully; quitting AppV2.", "INFO"
        )
        self.close()
